...
import { isArray, get, isEqual, orderBy, some, every, toLower } from "lodash";
...
const RowSeparator = ({ columns = [], hideOrShowColumn = () => {} }) => {
  const [initial, setInitial] = useState(false);
  const getValueFromField = (data, name) => {
    if (initial) {
      hideOrShowColumn(name, data);
    }
  };
  return (
    <StyledRowSeparator>
                    ...
                    !get(field, "root") && (
                      <div key={index + 1} className="option">
                        <div className="d-flex">
                          <Icon size="sm" icon={`icon-${toLower(get(field, "type"))}`} />
