...
const CustomBody = ({ CustomFooterOfBody, submitHandling = () => "" }) => {
  ...
  return (
    <CustomBodyStyle className={"clockPicker__body__style"}>
      <div className="clockPicker__body__container">
        ...
        <SimpleBar className="clockPicker__body__minutes__container">
          {generateNumber(60).map((i, ind) => (
            <div
              key={ind}
              className={`clockPicker__body__minutes__item ${timeData.minutes === ind && "selected"}`}
              onClick={() => setTimeData({ ...timeData, minutes: ind })}
            >
              {ind}
            </div>
          ))}
...
export default memo(CustomBody);
