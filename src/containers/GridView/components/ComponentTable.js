...
const TableStyle = styled(SimpleBar)`
...
const ComponentTable = ({ t, data = [], update = () => {}, hasModal = {}, remove = () => {}, columns = [], redirect = {} }) => {
  ...
  const getColor = (cell) => {
    if (isObject(cell.column.color) && "colorCode" in cell.column.color && "name" in cell.column.color && isObject(cell.value)) {
      return (
        <ColorViewer color={get(cell, `value[${cell.column.color.colorCode}]`)} className={"colorViewer"}>
          {get(cell, `value[${cell.column.color.name}]`)}
        </ColorViewer>
      );
    }
    return <span>{cell.render("Cell")}</span>;
  ...
  return (
    <TableStyle length={columns.length} {...{ columnWidth: 200 }} className="grid-view-table">
      <div>
